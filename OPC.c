...
void OPC_Ident (OPT_Object obj)
{
	...
	if ((__IN(mode, 0x62) && level > 0) || __IN(mode, 0x14)) {
		OPM_WriteStringVar((void*)obj->name, 32);
		h = OPC_PerfectHash((void*)obj->name, 32);
		...
		if (mode != 5 || obj->linkadr != 2) {
			if (mode == 13) {
				...
				OPM_WriteStringVar((void*)OPT_GlbMod[__X(-level, 64)]->name, 32);
				if (OPM_currFile == 0) {
	...
	if (((typ->strobj == NIL || typ->strobj->name[0] == 0x00) && typ->comp != 4)) {
		if (__IN(typ->comp, 0x0c)) {
			...
			*openClause = typ->comp == 2;
			...
			pointers = 0;
			while ((typ->strobj == NIL && typ->form == 13)) {
				pointers += 1;
				typ = typ->BaseTyp;
			}
			if (typ->comp != 3) {
...
static BOOLEAN OPC_Undefined (OPT_Object obj)
{
	return (((obj->mnolev >= 0 && obj->linkadr != (LONGINT)(3 + OPM_currFile))) && obj->linkadr != 2) || obj->name[0] == 0x00;
}
...
static void OPC_DeclareParams (OPT_Object par, BOOLEAN macro)
{
	...
	while (par != NIL) {
		if (macro) {
			OPM_WriteStringVar((void*)par->name, 32);
		} else {
...
void OPC_TDescDecl (OPT_Struct typ)
{
	...
	OPM_Write('\"');
	if (typ->strobj != NIL) {
		OPM_WriteStringVar((void*)typ->strobj->name, 32);
	}
...
LONGINT OPC_Base (OPT_Struct typ)
{
	switch (typ->form) {
		case 1: 
			...
			break;
		case 3: 
			return OPM_CharAlign;
			break;
		case 2: 
			return OPM_BoolAlign;
			break;
		case 4: 
			return OPM_SIntAlign;
			break;
		case 5: 
			return OPM_IntAlign;
			break;
		case 6: 
			return OPM_LIntAlign;
			break;
		case 7: 
			return OPM_RealAlign;
			break;
		case 8: 
			return OPM_LRealAlign;
			break;
		case 9: 
			return OPM_SetAlign;
			break;
		case 13: 
			return OPM_PointerAlign;
			break;
		case 14: 
			return OPM_ProcAlign;
			break;
		case 15: 
			if (typ->comp == 4) {
				return __MASK(typ->align, -65536);
			} else {
				return OPC_Base(typ->BaseTyp);
			}
			break;
		default: __CASECHK;
	}
...
static void OPC_IncludeImports (OPT_Object obj, INTEGER vis)
{
	if (obj != NIL) {
		...
		if ((((obj->mode == 11 && obj->mnolev != 0)) && (int)OPT_GlbMod[__X(-obj->mnolev, 64)]->vis >= vis)) {
			OPC_Include(OPT_GlbMod[__X(-obj->mnolev, 64)]->name, 32);
		}
...
static void OPC_GenHeaderMsg (void)
{
	...
	OPM_Write(' ');
	OPM_WriteString(version_versionLong, 256);
	OPM_Write(' ');
	_for__38 = OPM_MaxSet;
	i = 0;
	while (i <= _for__38) {
		if (__IN(i, OPM_glbopt)) {
			switch (i) {
				...
				case 16: 
					...
					break;
				default: 
					OPM_WriteString((CHAR*)"( more options defined in OPM than checked in OPC.GenHeaderMsg, if you are a compiler developer, add them to OPC.GenHeaderMsg", (LONGINT)126);
					break;
			}
		}
		i += 1;
...
static void OPC_InitImports (OPT_Object obj)
{
	if (obj != NIL) {
		...
		if ((obj->mode == 11 && obj->mnolev != 0)) {
			OPC_BegStat();
			OPM_WriteString((CHAR*)"__IMPORT(", (LONGINT)10);
			OPM_WriteStringVar((void*)OPT_GlbMod[__X(-obj->mnolev, 64)]->name, 32);
			OPM_Write(')');
			OPC_EndStat();
		}
...
void OPC_EnterBody (void)
{
	...
	if (OPC_mainprog) {
		if (OPC_ansi) {
			OPM_WriteString((CHAR*)"main(int argc, char **argv)", (LONGINT)28);
			OPM_WriteLn();
		} else {
...
void OPC_DefineInter (OPT_Object proc)
{
	...
	OPM_WriteString((CHAR*)"static ", (LONGINT)8);
	OPM_WriteString((CHAR*)"struct ", (LONGINT)8);
	OPM_WriteStringVar((void*)scope->name, 32);
	OPM_Write(' ');
	OPC_BegBlk();
...
void OPC_ExitProc (OPT_Object proc, BOOLEAN eoBlock, BOOLEAN implicitRet)
{
	...
	if ((implicitRet && proc->typ != OPT_notyp)) {
		...
		if (!proc->scope->leaf) {
			...
			OPM_WriteStringVar((void*)proc->scope->name, 32);
			OPM_WriteString((CHAR*)"_s", (LONGINT)3);
			OPM_WriteString((CHAR*)" = ", (LONGINT)4);
			OPM_WriteString((CHAR*)"_s", (LONGINT)3);
			OPM_Write('.');
...
void OPC_CompleteIdent (OPT_Object obj)
{
	...
	if (obj->adr == 1) {
		...
		OPM_WriteStringVar((void*)obj->scope->name, 32);
		OPM_WriteString((CHAR*)"_s", (LONGINT)3);
		OPM_WriteString((CHAR*)"->", (LONGINT)3);
		OPC_Ident(obj);
	} else {
...
void OPC_TypeOf (OPT_Object ap)
{
	...
	if (ap->mode == 2) {
		if ((int)ap->mnolev != OPM_level) {
			OPM_WriteStringVar((void*)ap->scope->name, 32);
